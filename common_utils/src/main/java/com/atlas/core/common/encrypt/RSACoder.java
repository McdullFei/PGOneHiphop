package com.atlas.core.common.encrypt;
...
/**
 *
 * 与客户端通信的非对称加密,可以针对session动态获取公私钥,也可全局保存一份
 *
 *
 * @author renfei
 *
...
public class RSACoder {
    ...
    private static Map<String, Object> initKey() throws Exception {
        ...
        keyMap.put("PB_KEY", publicKey);
        keyMap.put("PV_KEY", privateKey);
        return keyMap;
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        byte[] bytes = encryptByPublicKey(t.getBytes(), getPublicKey());

//        String bytes1 = Base64.encodeBase64String(bytes);
        byte[] bytes1 = Base64.encodeBase64(bytes);

        String c = new String(bytes1, "utf-8");

        System.out.println("加密后的数据: " + c);

        byte[] bytes2 = Base64.decodeBase64(c.getBytes());

        byte[] bytes3 = decryptByPrivateKey(bytes2, getPrivateKey());
